...
- Use `CUDA_DEVICE=0,1` and `learner_gpus=2` by default for performance and
...
- `RiichiEnv/riichienv-state-machine/` is the MJAI protocol state-transition and persistence
...
- Default performance and training tests must explicitly use
  `target_kl=0.0`, `update_epochs=4`, and `kyokus_per_worker=16`. This test
