...
def test_target_kl_zero_completes_all_ppo_epochs() -> None:
    learner = PPOLearner("mid", "cpu", learning_rate=1e-4, profile_enabled=False,
        profile_cuda_sync=False, update_epochs=2, minibatch_size=2, ppo_clip=0.2,
        value_coef=0.5, entropy_coef=0.01, max_grad_norm=0.5, target_kl=0.0)
...
def test_cpu_update_keeps_fp32_parameters_and_disables_bf16_autocast() -> None:
    learner = PPOLearner("mid", "cpu", learning_rate=1e-4, profile_enabled=False,
        profile_cuda_sync=False, update_epochs=1, minibatch_size=1, ppo_clip=0.2,
        value_coef=0.5, entropy_coef=0.01, max_grad_norm=0.5, target_kl=0.0)
    assert not learner.use_bf16
    assert {parameter.dtype for parameter in learner.model.parameters()} == {torch.float32}


def test_unmarked_v4_checkpoint_is_rejected() -> None:
    learner = PPOLearner("mid", "cpu", learning_rate=1e-4, profile_enabled=False,
        profile_cuda_sync=False, update_epochs=1, minibatch_size=1, ppo_clip=0.2,
        value_coef=0.5, entropy_coef=0.01, max_grad_norm=0.5, target_kl=0.0)
    with TemporaryDirectory() as directory:
        path = f"{directory}/v4.pt"
        torch.save({"model": {}}, path)
        try:
            learner.load(path)
        except ValueError as error:
            assert "Token V5" in str(error)
        else:  # pragma: no cover
            raise AssertionError("unmarked V4 checkpoint must be rejected")
