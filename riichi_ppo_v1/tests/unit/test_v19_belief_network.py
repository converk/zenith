"""V19 信念网络模块单测：模糊化五头形状、摘要拼接、token 形状与共享矩阵、Loss 范围。

V19 60% 方案起，``BeliefNetwork`` 接收 backbone 读出的
``player_query_hidden [B,3,3,256]``；五个头共享逐家小头、对每个查询分别
...
def test_belief_summary_width_and_composition() -> None:
    """摘要为 130 维且逐段拼接顺序与设计一致。"""
    output = _belief_output()
    summary = output["belief_summary"]
    assert summary.shape == (2, 3, 130)
    # softmax(hand)48 + softmax(shanten)9 + softmax(wait)5 + sigmoid(danger)34
    # + loss_pred(已 sigmoid)34。
    ...
    assert summary.shape[-1] == 16 * 3 + 9 + 5 + 34 + 34
...
def test_token_matrix_shared_between_players() -> None:
    """三家共用同一个转换矩阵：矩阵为单个 130→10×d_model 的 Linear。"""
    network = BeliefNetwork()
    matrix = network.token_matrix
    assert isinstance(matrix, torch.nn.Linear)
    assert matrix.in_features == 130
