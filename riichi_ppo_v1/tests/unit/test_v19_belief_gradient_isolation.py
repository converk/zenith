...
1. 策略/BC 损失只能更新 ``token_matrix``，不得回传信念四头、belief backbone 与
   belief_query；
2. 信念网络（四头 + 1 层 backbone + belief_query）的梯度完全来自四头监督标签；
...
def _belief_heads_have_no_grad(model: KyokuTransformerActorCritic) -> bool:
    """信念四头、belief backbone 与 belief_query 均无梯度（token_matrix 可除外）。"""
...
def test_fuzzy_wait_loss_is_categorical() -> None:
    ...
    # loss 头为分桶 CE（四头之一）。
    assert "belief/loss_bucket_loss" in parts
    assert "belief/loss_bucket_loss_norm" in parts


def test_supervised_loss_updates_belief_network_only_path() -> None:
    """四头监督损失更新信念网络，且不经过 token_matrix。"""
    model, inputs = _model_and_inputs()
    output = _forward_policy_only(model, inputs, belief_readout_detach=True)
    batch = {
        "belief_hand": torch.randint(0, 3, (2, 48), dtype=torch.long),
    ...
    # 四头、backbone 与 belief_query 都从监督路径获得梯度。
    for name in (
        "belief_network.hand_head.weight",
        "belief_network.wait_head.weight",
        "belief_network.danger_head.weight",
