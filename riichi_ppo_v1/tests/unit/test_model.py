...
def test_value_loss_does_not_update_public_actor_backbone() -> None:
    torch.manual_seed(7)
    model = KyokuTransformerActorCritic(config())
    factors, numeric = semantic_token_inputs(1, 3)
    output = model(
    ...
    assert all(
        ...
        if name.startswith(("token_embedding", "backbone", "query", "policy_head"))
    )
    assert any(
        parameter.grad is not None
        for name, parameter in model.named_parameters()
        if name.startswith(("critic_embedding", "critic_backbone", "value_query", "value_head"))
    )
