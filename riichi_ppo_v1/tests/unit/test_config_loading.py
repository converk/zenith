...
class ConfigLoadingTest(unittest.TestCase):
    def test_packaged_groups_provide_the_complete_default_configuration(self) -> None:
        ...
        self.assertEqual(config["kyokus_per_worker"], 32)
        self.assertEqual(config["update_batch_mode"], "streaming")
        self.assertEqual(config["gae_lambda"], 0.95)
        self.assertEqual(config["target_kl"], 0.03)
        self.assertEqual(config["learning_rate"], 0.00025)
        self.assertEqual(config["total_updates"], 10000)
        self.assertEqual(config["warmup_fraction"], 0.02)
        self.assertEqual(config["entropy_start"], 0.01)
        self.assertEqual(config["entropy_end"], 0.004)
        ...
        self.assertEqual(config["checkpoint_dir"], "checkpoints/riichi_ppo_v1")
