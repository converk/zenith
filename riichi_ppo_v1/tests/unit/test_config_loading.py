...
class ConfigLoadingTest(unittest.TestCase):
    def test_packaged_groups_provide_the_complete_default_configuration(self) -> None:
        ...
        self.assertEqual(config["checkpoint_dir"], "checkpoints/train_riichi_v8")
        ...
        self.assertIsNone(config["resume"])
        self.assertEqual(config["initial_discard_weight"], 0.25)
        self.assertEqual(config["initial_call_weight"], 0.10)
        self.assertEqual(config["opponent_pool_capacity"], 8)
        self.assertEqual(config["opponent_resident_models"], 2)
