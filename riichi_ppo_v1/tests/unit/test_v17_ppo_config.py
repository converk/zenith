"""V17 PPO 自包含配置断言:512 半庄/update、3072 global batch、评测节奏等。"""
...
def test_v17_config_is_self_contained() -> None:
    ...
    # 512 半庄/update(rollout 停止条件为完整半庄数)。
    assert config["games_per_update"] == 512
    ...
    assert config["entropy_end"] == 0.005
    assert config["critic_bootstrap_updates"] == 2
    assert config["total_updates"] == 100
    # Q-Boosting。
    assert config["q_boost_coef"] == 0.05
    assert config["q_boost_lambda"] == 1.0
    assert config["q_temperature"] == 1.5
    assert config["top3_q_candidates"] == 3
    # SFT KL。
    assert config["sft_kl_coef_start"] == 0.002
    assert config["sft_kl_coef_middle"] == 0.0005
    assert config["sft_kl_coef_end"] == 0.001
    # 对手纯 current self-play。
    assert config["opponent_mix"]["enabled"] is False
    # 评测节奏:每 5 updates,4000 半庄。
