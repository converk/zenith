...
本文定义 RiichiEnv、MJAI 状态机、Python bridge 与 V19 模型的状态边界。动作空间和
MJAI 回转见 [KyokuActionSpace.md](KyokuActionSpace.md)，精确张量/schema 见
[v19_input_protocol.md](v19_input_protocol.md)。
...
V19 **不再把 MJAI 事件序列编码进模型输入**。事件只用于状态机同步、牌局推进、奖励与
动作执行（`new_events()`/`apply_events_batch`）；模型输入由当前 `Observation` 的快照
字段直接构造，信念 token 由模型内部生成。每张桌维护四个观察者状态机。
...
1、2、3 的真实闭手，最后是 value query；不包含未来牌山。任何 private token 都不能
改变 Actor raw logits；Critic 不接收 Opponent Analysis、Action Query 或信念 token。
...
旧代次输入协议文档已随全仓清理移除（仅存于 git 历史）；活跃代码不提供旧契约的
加载、迁移或协议适配。
