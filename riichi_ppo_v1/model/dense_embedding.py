...
class DenseSlotFusion(nn.Module):
    """独立类别融合（保留 API；生产模型使用共享投影路径）。"""

    def __init__(
        self,
        schema: CategorySchema,
        d_model: int,
        fusion_dim: int = DENSE_FUSION_DIM,
        slot_dim: int = DENSE_SLOT_DIM,
        ...
        self.discrete = nn.ModuleList(
            nn.Embedding(field.cardinality, self.slot_dim, padding_idx=0)
            for field in schema.discrete
        )
        self.numeric = nn.ModuleList(
            nn.Linear(1, self.slot_dim, bias=False) for _field in schema.numeric
        )
        if schema.slot_count:
            self.slot_id = nn.Embedding(schema.slot_count + 1, self.slot_dim, padding_idx=0)
        else:
            self.slot_id = None
        ...
        self.norm = nn.RMSNorm(self.fusion_dim)
        for table in self.discrete:
            nn.init.normal_(table.weight, std=1.0 / math.sqrt(self.slot_dim))
            if table.padding_idx is not None:
                with torch.no_grad():
                    table.weight[table.padding_idx].zero_()
        if self.slot_id is not None:
            nn.init.normal_(self.slot_id.weight, std=1.0 / math.sqrt(self.slot_dim))
            with torch.no_grad():
                self.slot_id.weight[0].zero_()

    def forward(self, factors: Tensor, numeric: Tensor) -> Tensor:
        squeeze = factors.ndim == 2
        if squeeze:
            factors = factors[None]
            numeric = numeric[None]
        fields = [
            table(factors[..., 2 + index].clamp(0, table.num_embeddings - 1))
            for index, table in enumerate(self.discrete)
        ]
        numeric_parts = [
            self.numeric[index](_clip_normalize(numeric[..., index], field.scale).unsqueeze(-1))
            for index, field in enumerate(self.schema.numeric)
        ]
        if self.schema.slot_count:
            slot_fields = fields[-4 * self.schema.slot_count:]
            scalar = fields[: len(fields) - 4 * self.schema.slot_count]
        ...
        projected = self.norm(self.projection(fused))
        output = projected if self.shared_mlp is None else self.shared_mlp(projected)
