...
def test_safe_fallback_prefers_pass_over_unmatched_model_action() -> None:
    ...
    prepared = PreparedDecision(
        ...
        seat=1,
        history_factors=np.zeros((0, 10), dtype=np.uint8),
        history_numeric=np.zeros((0, 8), dtype=np.float32),
        history_length=0,
        snapshot_kinds=np.zeros((0,), dtype=np.uint8),
        snapshot_cat=np.zeros((0, 4), dtype=np.uint8),
        snapshot_num=np.zeros((0, 7), dtype=np.float32),
        snapshot_length=0,
        query_rows=np.zeros((0, 15), dtype=np.int32),
        query_action_ids=np.zeros((0,), dtype=np.int32),
        query_pair_count=0,
