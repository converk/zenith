...
3. Codex 会先读计划与调研文件，然后按 E0→E1→E2→E3→E4（→可选 E5）连续执行；期间用 `/goal` 查看状态，`/goal pause` / `/goal resume` / `/goal clear` 控制运行。
...
迭代策略：每完成一个实验，先按 EXPERIMENT_PLAN.md 的判定标准给出结论（成功/部分成功/失败）并更新 PROGRESS.md，再决定进入下一个实验或做一次针对性调整；E3 必须在 E1/E2 之后；E5 仅在 E1–E4 均未达标时启动。
